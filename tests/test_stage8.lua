--[[
    ...
    resolution scaling: stretch/fit mapping, hit-test, events.
]]
...
-- ---------------------------------------------------------------------
-- Text engine: measure (monospace outside MTA)
-- ---------------------------------------------------------------------
...
-- cache: re-measure returns same values (no stale)
...
-- hard word-break: one word wider than wrapWidth
...
-- ---------------------------------------------------------------------
-- Label: wrap / align / valign / shadow via render items
-- ---------------------------------------------------------------------
local items = {}
local mock = {
...
-- shadow: two items per line (shadow + main), shadow offset
items = {}
...
-- Button: centered text by default
items = {}
...
-- events carry design coords (aligned with worldX)
local evX = nil
node:on("mousedown", function(e) evX = e.x end)
ui:onMouseDown(30, 60, "left")
eq(evX, 15, "design: event x in design space (30/2)")

-- layout in design space: root children get design size
...
-- ---------------------------------------------------------------------
DXUI.setDesignResolution(300, 300, "fit") -- screen 800x600: s=2, offX=(800-600)/2=100
ui:setScreenSize(800, 600)
items = {}
...
-- hit-test through letterbox offset
local fitClicked = false
fitNode:on("click", function() fitClicked = true end)
ui:onMouseDown(130, 30, "left") -- design ((130-100)/2, (30-0)/2) = (15,15) inside
