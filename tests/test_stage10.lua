--[[
    ...
    Tests: opacity (inherited alpha modulation: values, inheritance,
    zero early-out, animation), RT effect layer (node-level
    blur/mask via rtgroup: emission, offset compositing, fallback without RT).
]]

dofile("loader.lua")

-- Fake dx functions (effects lazy — inject before first use)
...
-- ---------------------------------------------------------------------
-- Mock backend with RT-group semantics (mirrors backend_mta: offset stack)
-- ---------------------------------------------------------------------
local calls = {}
local groupStack = {}
...
-- inheritance: parent 0.5 × child 0.5 = 0.25
...
-- opacity 0: node fully transparent — items not emitted (early-out)
child.opacity = 0
calls = {}
ui:renderFrame()
...
-- opacity animation: path midpoint modulates color
local now = 0
...
-- ---------------------------------------------------------------------
-- RT effect layer: node-level blur on Panel
-- ---------------------------------------------------------------------
local bp = ui:panel({ x = 100, y = 100, width = 80, height = 40, color = 0xFF204060, blur = 2 })
calls = {}
ui:renderFrame()

-- emission: rtgroup with blur effect, panel rect inside
local group = nil
...
-- content drawn in LOCAL coords (offset subtracted)
local inner = nil
for i = 1, #calls do
...
-- ---------------------------------------------------------------------
-- RT mask on Panel (for rect widgets)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Fallback: RT unavailable → plain render without group
-- ---------------------------------------------------------------------
dxCreateRenderTarget = nil -- take away RT (canGroup → false)
