--[[
    ...
    ScrollPanel: scrollable container (virtualization later).

    Composite (single object for the user):
      * viewport — the node itself (clip=true, visual bounds);
      * content  — invisible container (plain Node, no render), positioned
                   at (-scrollX, -scrollY); user children go here;
      * track/thumb — scrollbars (optional).

    Scrolling shifts content; children slide out through the viewport's
    clip region (geometric clip). The tree is not rebuilt.

    props: axis ("v"|"h"|"both", default "v"), scrollbar (bool), wheelStep,
           contentW/contentH (explicit size), children (into content).
]]

DXUI = DXUI or {}

local TRACK_W = 8    -- vertical track width / horizontal track height
local THUMB_MIN = 20 -- minimum thumb size
local WHEEL_STEP = 40
...
-- ---------------------------------------------------------------------
-- Internal: content size / scrollbar layout
-- ---------------------------------------------------------------------
...
--- Explicit content size (nil = auto-measure from children).
...
--- Content node (for manually attaching children).
...
--- Override setSize: relayout scrollbars + clamp scroll.
...
--- Builder: ui:scrollpanel({ axis=, scrollbar=, wheelStep=, contentW=,
-- contentH=, children=, ... }).
function ScrollPanel.build(context, props)
    ...
    -- content: invisible container (plain Node, no render)
    ...
    -- wheel (bubbles from content children to viewport)
