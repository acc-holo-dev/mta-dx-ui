--[[
    ...
    ComboBox: a dropdown list. node (a button with label + arrow) + dropdown
    (popup). items are strings or {text=, value=}. onChange(idx, value).
]]
...
--- Drop the dropdown when destroyed (it is a root node — the cascade does not reach it).
...
--- Builder: ui:combobox({ items=, selected=, placeholder=, onChange=, ... }).
function ComboBox.build(context, props)
    ...
    -- dropdown is a separate popup (root node), hidden; outside-click dismiss comes free
