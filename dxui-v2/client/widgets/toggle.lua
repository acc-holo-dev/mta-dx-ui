--[[
    ...
    Toggle: общий базовый класс для CheckBox и RadioButton.
    box (рамка) + mark (галочка/точка) + label — рисуются в render.
    Клик переключает checked; onChange(checked) — колбэк.

    Подкласс переопределяет _drawMark(renderer, bx, by) — форма отметки.
]]
...
-- По умолчанию — квадратная галочка (CheckBox).
...
--- Подключить колбэк изменения (или через props.onChange).
