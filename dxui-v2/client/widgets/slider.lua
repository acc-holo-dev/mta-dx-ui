--[[
    ...
    Slider: ползунок. track + fill + thumb рисуются в render. mousedown
    ставит значение по позиции (click-to-jump) и захватывает drag; движение
    продолжает менять значение. orientation "h" (default) | "v".
]]
...
--- Переводит экранную позицию (по оси трека) в value.
...
--- Билдер: ui:slider({ value=, min=, max=, orientation=, onChange=, ... }).
function Slider.build(context, props)
    ...
    node:on("mousedown", function(e)
        ...
        -- drag продолжает менять значение (delta-подход от точки захвата)
