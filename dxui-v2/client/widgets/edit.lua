--[[
    ...
    Edit: однострочный/многострочный ввод (§50 — базовый функционал).
    Фокус — через dispatcher (mousedown автоматически). Ввод — "text" событие;
    специальные клавиши — "key" событие (MTA-имена).

    Состояние (internal, с префиксом _): _cursor (0-based), _selAnchor,
    _goalCol (вертикальная навигация), _hasFocus. Текст — свойство text
    (source of truth).

    Геометрия курсора/выделения — через text engine (§41 measure, кэш §43):
    в MTA — точные метрики шрифта (dxGetTextSize); высота строки — measured
    (fallback EDIT_LINE_H вне MTA). Выделение рисуется per-line: первая и
    последняя строка — частичные ректы, средние — полные. Drag-select (§75):
    движение с зажатой левой кнопкой расширяет выделение от точки захвата.

    Clipboard: внутренний буфер контекста + MTA setClipboard/getClipboard,
    если доступны (вне MTA — только внутренний, тестируемо).
]]
...
--- Текст строки row (по _rowCol-нумерации).
function Edit:_rowText(row)
    local lines = splitLines(self.text)
    return lines[row] or ""
end

--- Stage 11: пиксель колонки через text engine (§41 measure, кэш §43).
-- MTA: dxGetTextSize — точно по реальному шрифту; вне MTA — monospace.
...
--- Обратное: локальный пиксель x → колонка строки row (ближайшая по ширине).
...
--- Высота строки — по text engine (§41): точные метрики в MTA;
-- вне MTA без подмен — monospace-оценка (EDIT_LINE_H fallback).
...
--- Точка события (design-координаты) → (row, col). Общая геометрия
--- для клика по полю и drag-select.
...
--- (row, col) → 0-based позиция курсора в общем тексте.
...
function Edit:render(renderer)
    ...
    -- выделение: per-line ректы (первая/последняя строка — частичные,
    -- средние — полные); ширины через text engine measure (§41)
    if self._hasFocus then
        ...
        if b > a then
            ...
            if rowA == rowB then
                ...
                -- последняя строка: от начала до colB
                local x2 = self:_colX(rowB, colB)
...
function Edit:_handleKey(key, mods)
    ...
    if isCtrl then
        ...
        elseif key == "c" then
            ...
            if b > a then
                ...
                if setClipboard then setClipboard(s) end -- MTA: общий буфер ОС
            end
            return
        elseif key == "v" then
            -- MTA: общий буфер ОС (getClipboard); иначе — внутренний буфер
    ...
    -- escape (§50): покинуть поле — работает и в readonly
    if key == "escape" then
    ...
    elseif key == "arrow_u" or key == "arrow_d" then
        -- вертикальная навигация (multiline): запоминаем целевую колонку
        -- (_goalCol), чтобы «змейка» между строками разной длины работала
        local row, col = self:_rowCol(self._cursor)
        local lines = splitLines(self.text)
...
function Edit:setCursor(pos)
    ...
    self._selAnchor = self._cursor
    -- позиция курсора — render-состояние: без инвалидации курсор «залипнет»
...
--- Билдер: ui:edit({ text=, placeholder=, readonly=, multiline=, maxLength=,
-- onChange=, onEnter=, ... }).
function Edit.build(context, props)
    ...
    node:on("mousedown", function(e)
        ...
        -- курсор по позиции клика: строка по Y, колонка через text engine
        local row, col = node:_posToRowCol(e.x, e.y)
        node._cursor = node:_rowColToCursor(row, col)
        node._selAnchor = node._cursor
        node:_invalidate({ DXUI.DIRTY.RENDER })
        -- drag-select (§75): движение с зажатой кнопкой расширяет выделение
        -- (_selAnchor фиксирован в точке захвата, двигается _cursor)
    ...
    -- фокус/блюр меняют видимость курсора — render-состояние (§26)
