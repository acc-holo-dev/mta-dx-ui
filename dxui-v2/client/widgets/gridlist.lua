--[[
    ...
    GridList: таблица. columns = { {text=, width=}, ... }. addColumn/addRow/
    selectRow/getSelected/getRowCount/clearRows. Скролл — встроенный ScrollPanel.
]]
...
function GridList:_rebuildHeader()
    -- заголовки — labels-дети узла header
...
--- Билдер: ui:gridlist({ columns=, rowHeight=, headerHeight=, onSelect=, ... }).
function GridList.build(context, props)
    props = props or {}
    local node = GridList:new(props)
    -- addColumn/_rebuildHeader создают labels через контекст до монтирования
