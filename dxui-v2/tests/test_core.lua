--[[
    ...
    Проверяет ядро: Node (property-style + method-style), инвалидацию,
    parent/child, lifecycle, наследование, цвет.
]]

-- Загрузка модулей в порядке meta.xml
dofile("loader.lua")
...
-- ---------------------------------------------------------------------
-- Property-style + method-style сходятся в один mutation layer
-- ---------------------------------------------------------------------
...
-- userData — без инвалидации, произвольные данные
node.userData = { itemId = 125, slot = 7 }
eq(node.userData.itemId, 125, "userData arbitrary")

-- произвольное пользовательское поле (не в spec) — rawset
...
-- destroyed node: запись — no-op (не падает)
...
-- метод базового класса доступен через цепочку
btn:setPosition(1, 2)
